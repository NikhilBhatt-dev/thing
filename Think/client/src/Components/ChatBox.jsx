...
const ChatBox = () => {
  ...
  return (
    <div className='flex-1 flex flex-col justify-between m-5 md:m-10 xl:mx-30 max-md:mt-14 2xl:pr-40'>
      ...
      {/* prompt input box */}
      <form onSubmit={onSubmit} className='bg-primary/20 dark:bg-[#583C79]/30 border-primary dark:border-[#80609F]/30 rounded-full max-w-2xl p-3 pl-4 mx-auto flex gap-4 items-center mt-5'>
        <select onChange={(e) => setMode(e.target.value)} value={mode} className='text-sm pl-3pr-2 outline-none' id="">
          <option value="text"  className='dark:bg-purple-900'>Text</option>
          <option value="image" className='dark:bg-purple-900'>Image</option>
        </select>
        <input onChange={(e)=>setPrompt(e.target.value)} type="text" placeholder='Ask anything'
         className='flex-1 w-230 text-sm outline-none ' required/>
